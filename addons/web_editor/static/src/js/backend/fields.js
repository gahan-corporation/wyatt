gerp.define('web_editor.backend', function (require) {
'use strict';
...
var FieldTextHtml = AbstractField.extend({
    ...
    start: function () {
        ...
        window.gerp[this.callback+"_editor"] = function (EditorBar) {
            setTimeout(function () {
                self.on_editor_loaded(EditorBar);
            },0);
        };
        window.gerp[this.callback+"_content"] = function () {
            self.on_content_loaded();
        };
        window.gerp[this.callback+"_updown"] = null;
        window.gerp[this.callback+"_downup"] = function () {
            self.resize();
        };
    ...
    on_editor_loaded: function (EditorBar) {
        ...
        if (this.value && window.gerp[self.callback+"_updown"] && !(this.$content.html()||"").length) {
            this.render();
        }
        setTimeout(function () {
            setTimeout(self.resize,0);
    ...
    render: function () {
        ...
        if (this.mode === "edit") {
            if (window.gerp[this.callback+"_updown"]) {
                // FIXME
                // window.gerp[this.callback+"_updown"](value, this.view.get_fields_values(), this.name);
                this.resize();
            }
        } else {
    ...
    destroy: function () {
        ...
        delete window.gerp[this.callback+"_editor"];
        delete window.gerp[this.callback+"_content"];
        delete window.gerp[this.callback+"_updown"];
        delete window.gerp[this.callback+"_downup"];
    },
