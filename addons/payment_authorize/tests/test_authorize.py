...
from gerp.addons.payment.tests.common import PaymentAcquirerCommon
from gerp.addons.payment_authorize.controllers.main import AuthorizeController
from gerp.tools import mute_logger


@gerp.tests.common.at_install(True)
...
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('gerp.addons.payment_authorize.models.payment', 'ValidationError')
    ...
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on gerp/gerp + create sandbox account
        authorize.write({
        ...
        transaction = self.env['payment.transaction'].create({
            ...
            'type': 'server2server',
            'currency_id': self.currency_usd.id,
            'reference': 'test_ref_%s' % gerp.fields.Date.today(),
