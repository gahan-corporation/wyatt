...
import gerp
import gerp.tools as tools
import gerp.release as release
from gerp import SUPERUSER_ID, api
...
class AddonsHook(object):
    """ Makes modules accessible through openerp.addons.* and gerp.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('gerp.addons.', 'openerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        gerp_name = re.sub(r'^openerp.addons.(\w+)$', r'gerp.addons.\g<1>', name)
        openerp_name = re.sub(r'^gerp.addons.(\w+)$', r'openerp.addons.\g<1>', gerp_name)

        assert gerp_name not in sys.modules
        ...
        # create empty gerp.addons.* module, set name
        new_mod = types.ModuleType(gerp_name)
        ...
        setattr(gerp.addons, addon_name, new_mod)
...
class GerpHook(object):
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'gerp\g<1>', name)
...
def initialize_sys_path():
    ...
    ``import gerp.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add gerp.addons.__path__
    for ad in __import__('gerp.addons').addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = gerp.tools.osutil.listdir(path, True)
...
def load_openerp_module(module_name):
    ...
    initialize_sys_path()
    try:
        __import__('gerp.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['gerp.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='gerp.tests'):
...
def run_unit_tests(module_name, dbname, position=runs_at_install):
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, gerp.sql_db.sql_counter - t0_sql)
