...
import gerp
import gerp.modules.db
import gerp.modules.graph
import gerp.modules.migration
import gerp.modules.registry
import gerp.tools as tools

from gerp import api, SUPERUSER_ID
from gerp.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('gerp.tests')
...
def load_module_graph(cr, graph, status=None, perform_checks=True, skip_modules=None, report=None):
    ...
    def load_test(module_name, idref, mode):
        ...
        try:
            ...
            if tools.config.options['test_commit']:
                ...
                gerp.registry(cr.dbname).clear_caches()
    ...
    migrations = gerp.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if new_install:
            py_module = sys.modules['gerp.addons.%s' % (module_name,)]
        ...
        if hasattr(package, 'init') or hasattr(package, 'update') or package.state in ('to install', 'to upgrade'):
            ...
            overwrite = gerp.tools.config["overwrite_existing_translations"]
            ...
            if has_demo:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(gerp.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, gerp.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    cr = db.cursor()
    try:
        if not gerp.modules.db.is_initialized(cr):
        ...
        # gerp.modules.registry.Registry.new().
        registry = gerp.registry(cr.dbname)
        ...
        graph = gerp.modules.graph.Graph()
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['gerp.addons.%s' % (pkg.name,)]
                ...
                return gerp.modules.registry.Registry.new(cr.dbname, force_demo, status, update_module)
