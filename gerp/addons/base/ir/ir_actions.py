...
from gerp.exceptions import MissingError, UserError, ValidationError, AccessError
from gerp.tools.safe_eval import safe_eval, test_python_expr
...
class IrActionsServer(models.Model):
    ...
    Since Gerp 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: Gerp Environment on which the action is triggered
#  - model: Gerp Model of the record on which the action is triggered; is a void recordset
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': gerp.exceptions.Warning,
            # record
            'record': record,
            'records': records,
            # helpers
