...
import gerp
from gerp import api, http, models, tools, SUPERUSER_ID
from gerp.exceptions import AccessDenied, AccessError
from gerp.http import request, STATIC_CACHE, content_disposition
...
class IrHttp(models.AbstractModel):
    ...
    def routing_map(cls):
        if not hasattr(cls, '_routing_map'):
            ...
            if tools.config['test_enable'] and gerp.modules.module.current_test:
                installed.add(gerp.modules.module.current_test)
            mods = [''] + gerp.conf.server_wide_modules + sorted(installed)
