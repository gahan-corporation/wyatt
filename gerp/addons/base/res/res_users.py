...
from gerp.exceptions import AccessDenied, AccessError, UserError, ValidationError
from gerp.osv import expression
from gerp.service.db import check_super
from gerp.tools import partition, pycompat
...
class Users(models.Model):
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Gerp (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: gerp.exceptions.AccessDenied when old password is wrong
        :raise: gerp.exceptions.UserError when new password is not set or empty
        """
