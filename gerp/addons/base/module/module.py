...
import gerp
from gerp import api, fields, models, modules, tools, _
from gerp.exceptions import AccessDenied, UserError
from gerp.tools.parse_version import parse_version
from gerp.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'Gerp Enterprise Edition License v1.0'),
        ('OPL-1', 'Gerp Proprietary License v1.0'),
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = gerp.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/gerp/addons directory to the new "gerp" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'gerp' subdirectory ?
                ...
                _logger.info('Copy downloaded module `gerp` to `%s`', server_dir)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.gerp.com/apps')
