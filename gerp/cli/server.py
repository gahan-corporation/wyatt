...
__author__ = gerp.release.author
__version__ = gerp.release.version

# Also use the `gerp` logger for the main script.
_logger = logging.getLogger('gerp')
...
def report_configuration():
    ...
    """
    config = gerp.tools.config
    _logger.info("Gerp version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', gerp.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = gerp.tools.config
    if not gerp.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = gerp.modules.registry.Registry.new(dbname)
        with gerp.api.Environment.manage():
            with registry.cursor() as cr:
                gerp.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with gerp.api.Environment.manage():
        with registry.cursor() as cr:
            gerp.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    gerp.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                gerp.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except gerp.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        gerp.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = gerp.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the gerp server (default command)"""
    def run(self, args):
        main(args)
