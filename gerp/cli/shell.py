...
class Shell(Command):
    """Start gerp in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        gerp.cli.server.report_configuration()
        gerp.service.server.start(preload=[], stop=True)
        ...
        with gerp.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = gerp.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = gerp.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
