# -*- coding: utf-8 -*-
"""
The module :mod:`gerp.tests.common` provides unittest test cases and a few
...
# The gerp library is supposed already configured.
ADDONS_PATH = gerp.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = gerp.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = gerp.SUPERUSER_ID


def get_db_name():
    db = gerp.tools.config['db_name']
...
class BaseCase(TreeCase):
    ...
    longMessage = True      # more verbose error message by default: https://www.gerp.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~gerp.models.BaseModel`
        """
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        self.registry = gerp.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        self.uid = gerp.SUPERUSER_ID
        #: :class:`~gerp.api.Environment` for the current test case
...
class SingleTransactionCase(BaseCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.registry = gerp.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.uid = gerp.SUPERUSER_ID
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = gerp.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        gerp.http.root.session_store.save(self.session)
    ...
    def authenticate(self, user, password):
        ...
        gerp.http.root.session_store.save(session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('gerp.service.http.request.'):
                join_retry_count = 10
