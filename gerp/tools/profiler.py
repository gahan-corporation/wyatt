...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if isinstance(in_self, (gerp.sql_db.Cursor, gerp.sql_db.TestCursor, gerp.sql_db.LazyCursor)):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all gerp models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non gerp model from the log: [None])
        ...
        .. code-block:: python
          ...
          from gerp.tools.profiler import profile
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all gerp models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/openerp/gerp/gerp/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _gerpProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_gerpProfile, method)

    wrapper = decorator(_gerpProfile, method)
    return wrapper
