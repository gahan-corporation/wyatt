...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except gerp.exceptions.except_orm:
        raise
    except gerp.exceptions.Warning:
        raise
    except gerp.exceptions.RedirectWarning:
        raise
    except gerp.exceptions.AccessDenied:
        raise
    except gerp.exceptions.AccessError:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except gerp.http.AuthenticationError:
        raise
    except OperationalError:
        # Do not hide PostgreSQL low-level exceptions, to let the auto-replay
        # of serialized transactions work its magic
        raise
    except gerp.exceptions.MissingError:
        raise
    except Exception as e:
        exc_info = sys.exc_info()
