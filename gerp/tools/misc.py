...
from gerp.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if gerp.tools.config['db_host']:
        env['PGHOST'] = gerp.tools.config['db_host']
    if gerp.tools.config['db_port']:
        env['PGPORT'] = str(gerp.tools.config['db_port'])
    if gerp.tools.config['db_user']:
        env['PGUSER'] = gerp.tools.config['db_user']
    if gerp.tools.config['db_password']:
        env['PGPASSWORD'] = gerp.tools.config['db_password']
    return env
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = gerp.modules.module.get_resource_path('base', 'res', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('gerp.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('gerp.foo.bar'):
            do_suff()
