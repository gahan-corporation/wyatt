...
import gerp.sql_db
import gerp.tools
from gerp.sql_db import db_connect
...
def check_super(passwd):
    if passwd and gerp.tools.config.verify_admin_password(passwd):
        return True
    raise gerp.exceptions.AccessDenied()

# This should be moved to gerp.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None):
    try:
        db = gerp.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            gerp.modules.db.initialize(cr)
            gerp.tools.config['load_language'] = lang
            cr.commit()

        registry = gerp.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = gerp.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = gerp.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = gerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = gerp.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    gerp.sql_db.close_db(db_original_name)
    db = gerp.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = gerp.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = gerp.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = gerp.tools.config.filestore(db_original_name)
    to_fs = gerp.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    gerp.modules.registry.Registry.delete(db_name)
    gerp.sql_db.close_db(db_name)
    ...
    fs = gerp.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'gerp_dump': '1',
        'db_name': cr.dbname,
        'version': gerp.release.version,
        'version_info': gerp.release.version_info,
        'major_version': gerp.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with gerp.tools.osutil.tempdir() as dump_dir:
            filestore = gerp.tools.config.filestore(db_name)
            ...
            gerp.tools.exec_pg_command(*cmd)
            if stream:
                gerp.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                gerp.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = gerp.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with gerp.tools.osutil.tempdir() as dump_dir:
        ...
        if gerp.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = gerp.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = gerp.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    gerp.modules.registry.Registry.delete(old_name)
    gerp.sql_db.close_db(old_name)
    ...
    old_fs = gerp.tools.config.filestore(old_name)
    new_fs = gerp.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    gerp.tools.config.set_admin_password(new_password)
    gerp.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        gerp.tools.config['update']['base'] = True
        gerp.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@gerp.tools.mute_logger('gerp.sql_db')
...
def list_dbs(force=False):
    if not gerp.tools.config['list_db'] and not force:
        raise gerp.exceptions.AccessDenied()
    ...
    with closing(db.cursor()) as cr:
        try:
            db_user = gerp.tools.config["db_user"]
            if not db_user and os.name == 'posix':
                import pwd
                db_user = pwd.getpwuid(os.getuid())[0]
            if not db_user:
                cr.execute("select usename from pg_user where usesysid=(select datdba from pg_database where datname=%s)", (gerp.tools.config["db_name"],))
                res = cr.fetchone()
            ...
            res = [gerp.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
            res = []
    res.sort()
    return res
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if gerp.tools.table_exists(cr, 'ir_module_module'):
                ...
                incompatible_databases.append(database_name)
        # release connection
        gerp.sql_db.close_db(database_name)
...
def exp_list_lang():
    return gerp.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(gerp.tools.config['root_path'], 'addons/base/res/res_country_data.xml')).getroot()
