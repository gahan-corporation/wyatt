...
# User code must use the exceptions defined in ``gerp.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, gerp.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, gerp.tools.ustr(e.value))
    ...
    elif isinstance(e, gerp.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = gerp.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, gerp.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(gerp.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(gerp.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, gerp is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(gerp.tools.ustr(value), write)
...
def wsgi_xmlrpc(environ, start_response):
    ...
    if environ['REQUEST_METHOD'] == 'POST' and environ['PATH_INFO'].startswith('/xmlrpc/'):
        ...
        try:
            result = gerp.http.dispatch_rpc(service, method, params)
...
def application_unproxied(environ, start_response):
    ...
    # gerp.service.web_services.objects_proxy.dispatch().
    ...
    with gerp.api.Environment.manage():
        ...
        for handler in [wsgi_xmlrpc, gerp.http.root]:
