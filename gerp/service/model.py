...
import gerp
from gerp.exceptions import UserError, ValidationError, QWebException
from gerp.models import check_method_name
from gerp.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = gerp.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = gerp.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if gerp.registry(dbname)._init and not gerp.tools.config['test_enable']:
                    raise gerp.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    recs = gerp.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return gerp.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with gerp.registry(db).cursor() as cr:
