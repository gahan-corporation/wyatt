...
This page introduces the new Gerp Coding Guidelines. Those aim to improve the quality of the code (e.g. better readability of source) and Gerp Apps. Indeed, proper code eases maintenance, aids debugging, lowers complexity and promotes reliability.
...
Don't link data (image, libraries) outside Gerp: do not use an
...
Gerp supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. Gerp
...
#. Imports of ``gerp``
#. Imports from Gerp modules (rarely, and only if necessary)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The Gerp framework is in charge of providing the transactional context for
...
Gerp uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in Gerp, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an Gerp Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an Gerp Transient (wizard) : use ``<related_base_model>.<action>``
...
- Gerp Python Class : use camelcase for code.
...
- Variable name :
    ...
    - Gerp works with a record or a recordset, don't suffix variable names with
...
- **[CLA]** for signing the Gerp Individual Contributor License
