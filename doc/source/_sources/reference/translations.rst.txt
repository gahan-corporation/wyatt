...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~gerp.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~gerp.models.Model._constraints` and
  :attr:`~gerp.models.Model._sql_constraints` are exported
...
In Python, the wrapping function is :func:`gerp._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`gerp.web._t`:
