...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects Gerp:
    ...
    ``context``
        merged into the view's context when performing the button's Gerp call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's Gerp call
...
Semantic components tie into and allow interaction with the Gerp
system. Available semantic components are:

``button``
  call into the Gerp system, similar to :ref:`list view buttons
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~gerp.fields.Char`,
    :class:`~gerp.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`gerp.api.onchange` on the model
  ...
  ``mode``
    for :class:`~gerp.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~gerp.fields.Char` field stores a password and
...
Most :class:`~gerp.fields.Many2many` fields, like categories, are better
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~gerp.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/gerp/gerp/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``read_only_mode``
    self-explanatory
...
``arrow`` (required, 1)
    ...
    ``object`` (required)
      the edge's Gerp model
    ``source`` (required)
      :class:`~gerp.fields.Many2one` field of the edge's model pointing to
...
``field``
    ...
    ``widget``
        ...
        standard Gerp 8.0 is a ``selection`` widget for
        :class:`~gerp.fields.Many2one` fields)
    ``domain``
        if the field can provide an auto-completion
        (e.g. :class:`~gerp.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain``
        an Gerp :ref:`domain <reference/orm/domains>`, will be appended to the
