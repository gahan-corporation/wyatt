...
Official compatibility: Gerp 11 will be the first LTS release to introduce
...
used by the official Gerp distribution have been ported and are considered
...
.. important::
    ...
    a guide to write 2/3-compatible Gerp code. It does not go through all the
    changes in Python but rather through issues which have been found in the
    standard Gerp distribution in order to show how to evolve such code such
...
A cross compatible Gerp would only support Python 2.7 and Python 3.5 and
...
.. warning::
    ...
    generator delegation, pathlib, ...), you must *not* use them in Gerp
...
* ``urllib``, ``urllib2`` and ``urlparse`` were redistributed across
  ...
  Since `requests`_ and `werkzeug`_ are already hard dependencies of Gerp,
...
* ``cgi.escape`` (HTML escaping) is deprecated in Python 3, prefer Gerp's own
...
actually happened in Gerp with :mod:`gerp.tools.mimetypes`.
...
* type-testing is the last and bigger issue as in Python 2 ``long`` is not a
  ...
  For that case, Gerp 11 now provides a compatibility module with an
...
However with respect to Gerp it turns out most or all uses of ``csv`` fit
