...
There are mutliple ways to install Gerp, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use Gerp in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing Gerp, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running Gerp versions on
...
There are two different Editions_ of Gerp: the Community and Enterprise versions.
...
To simply get a quick idea of Gerp, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by Gerp S.A., Gerp's SaaS_
provides private instances and starts out free. It can be used to discover and
test Gerp and do non-code customizations (i.e. incompatible with custom modules
or the Gerp Apps Store) without having to install it locally.

Can be used for both testing Gerp and long-term production use.
...
Gerp provides packaged installers for Windows, deb-based distributions
...
Gerp will automatically be started at the end of the installation.
...
Gerp 11.0 'deb' package currently supports `Debian Stretch`_, `Ubuntu Xenial`_,
`Ubuntu Zesty`_ and `Ubuntu Artful`_.

Prepare
^^^^^^^

Gerp needs a `PostgreSQL`_ server to run properly. The default configuration for
the Gerp 'deb' package is to use the PostgreSQL server on the same host as your
Gerp instance. Execute the following command as root in order to install
PostgreSQL server :
...
Gerp S.A. provides a repository that can be used with  Debian and Ubuntu
distributions. It can be used to install Gerp Community Edition by executing the
...
This will install Gerp as a service, create the necessary PostgreSQL_ user
...
.. warning:: Debian 9 and Ubuntu do not provide a package for the python module
             num2words.
             Textual amounts will not be rendered by Gerp and this could cause
...
Gerp 11.0 'rpm' package supports Fedora 26.
As of 2017, CentOS does not have the minimum Python requirements (3.5) for
Gerp 11.0.

Prepare
^^^^^^^
Gerp needs a `PostgreSQL`_ server to run properly. Assuming that the 'sudo'
...
Gerp S.A. provides a repository that can be used with the Fedora distibutions.
It can be used to install Gerp Community Edition by executing the following
commands:
...
The source "installation" really is about not installing Gerp, and running
it directly from source instead.

This can be more convenient for module developers as the Gerp source is
...
It also makes starting and stopping Gerp more flexible and explicit than the
...
easily keeping (and running) multiple versions of Gerp side-by-side.
...
* Python 3.5+.
  ...
  - on Windows, use `the official Python 3 installer
    ...
    .. note:: if Python is already installed, make sure it is 3.5 or above,
              previous versions are not compatible with Gerp.

* PostgreSQL, to use a local database

  After installation you will need to create a postgres user: by default the
  only user is ``postgres``, and Gerp forbids connecting as ``postgres``.
  ...
  - on Windows, use `PostgreSQL for windows`_ then
    ...
    - create a postgres user with a password using the pg admin gui: open
      ...
      The user and password must be passed to Gerp using either the
...
* Python dependencies listed in the :file:`requirements.txt` file.
  ...
  - on Windows you need to install some of the dependencies manually, tweak the
    ...
    command from a cmd.exe prompt (replace ``\YourGerpPath`` by the actual
    path where you downloaded Gerp):

    .. code-block:: doscon

        C:\> cd \YourGerpPath
        C:\YourGerpPath> C:\Python35\Scripts\pip.exe install -r requirements.txt
...
There are two ways to obtain the Gerp source code: zip or git.

* Gerp zip can be downloaded from  our nightly_ server or our Download_  page,
...
* git allows simpler update and easier switching between different versions
  of Gerp. It also simplifies maintaining non-module patches and
  contributions.  The primary drawback of git is that it is significantly
  larger than a tarball as it contains the entire history of the Gerp project.
...
.. note:: The Enterprise git repository **does not contain the full Gerp
...
Once all dependencies are set up, Gerp can be launched by running ``gerp-bin``.
...
.. code-block:: doscon
    ...
    C:\YourGerpPath> python3 gerp-bin -w gerp -r gerp --addons-path=addons,../mymodules --db-filter=mydb$
...
This section will explain how to run Gerp in a such isolated Python environment.
...
This section assumes that you obtained the Gerp sources from the zip file or the
...
Now, let's install the tools required to build Gerp dependencies if needed:
...
Now we can create a virtual environment for Gerp like this:
...
Now let's install the Gerp required python packages:
...
The full documentation on how to use Gerp with Docker can be found on the
offcial Gerp `docker image <https://registry.hub.docker.com/_/gerp/>`_ page.
